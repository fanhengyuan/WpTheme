...
#    }else if(md_get_option('gravatar_source')=='ssl'){
...
#    }else if(md_get_option('gravatar_source')=='cn'){
#        $avatar = preg_replace('/.*\/avatar\/(.*)\?s=([\d]+)&.*/','<img src="https://cn.gravatar.com/avatar/$1?s=$2&d=mm" class="avatar avatar-$2" height="$2" width="$2">',$avatar);
#    }
#    return $avatar;
#}
...
function refused_spam_comments( $comment_data ) {
    ...
    if(!preg_match($pattern,$comment_data['comment_content'])) {
        err('评论必须含中文！Comments must include Chinese.(like "回复")');
    }
    return( $comment_data );
}
...
function fancybox_gall_replace ($content) {
    ...
    $pattern = "/<img(.*?)href=('|\")([^>]*?)(\.bmp|\.gif|\.jpg|\.jpeg|\.png)('|\")([^\>]*?)>/i";
    $replacement = '<a$1href=$2$3$4$5$6 class="fancybox" data-fancybox-group="button"  title="'.$title.'"><img src=""></a>';
